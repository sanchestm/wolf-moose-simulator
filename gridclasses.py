...
class Grid:
    def __init__(self, contour, vegimage, twiimage, cityimage, pixelXmeters, larvaelim, gmotype, eqsteps):
        ...
        if os.path.exists("eqgridspkl/migdict_pxlsize" +str(pixelXmeters)+".p") == False:
            ...
            pkl.dump( [self.maxstep, self.dict_matrix_to_num, self.dict_num_to_weights], open( "eqgridspkl/migdict_pxlsize" +str(pixelXmeters)+".p", "wb" ) )
        else:
            print("migration dt = "+ str(self.maxstep/60))
            self.maxstep, self.dict_matrix_to_num, self.dict_num_to_weights = pkl.load(open("eqgridspkl/migdict_pxlsize" +str(pixelXmeters)+".p", "rb"))
        ...
        if os.path.exists("eqgridspkl/poplim"+str(larvaelim)+ "neq"+ str(eqsteps)+".p") == False:
            CURSOR_UP_ONE = '\x1b[1A'
            ERASE_LINE = '\x1b[2K'
            for i in range(eqsteps):
                print("loading equalizing: " +str(i) + " done")
                self.updateall()
                print(CURSOR_UP_ONE + ERASE_LINE + CURSOR_UP_ONE)
            self.internalclock = 0
            self.history = []
            pkl.dump( self.GRID, open( "eqgridspkl/poplim"+str(larvaelim)+ "neq"+ str(eqsteps)+".p", "wb" ) )
        else:
            self.GRID = pkl.load(open("eqgridspkl/poplim"+str(larvaelim)+ "neq"+ str(eqsteps)+".p", "rb"))
            for i in range(1, self.shape[0]-1):
                for j in range(1, self.shape[1]-1):
                    self.GRID[i][j].gmotype = gmotype
    ...
    def images(self):
        ...
        cao=ao.imshow(self.getSingleGrid('amut'), cmap=plt.get_cmap("gist_earth"),  vmin = 0, vmax = self.maxafem)
        ...
        caa=adults.imshow(self.getSingleGrid('adult'), cmap=plt.get_cmap("gist_earth"),  vmin = 0, vmax = 0.8*self.maxafem)
